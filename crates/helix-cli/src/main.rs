...
enum Commands {
    ...
    /// Plot-scoped file tools (list/read/write/delete under the active plot only)
    Files {
        #[command(subcommand)]
        cmd: FilesCmd,
    },
...
#[derive(Subcommand)]
enum FilesCmd {
    /// List entries under a plot-relative path (default: plot root)
    Ls {
        #[arg(default_value = "")]
        path: String,
        #[arg(long, default_value = "default")]
    ...
    /// Write a UTF-8 text file under the plot (content via --content or stdin)
    Write {
        path: String,
        #[arg(long)]

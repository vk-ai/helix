...
> Status: **early slice**. Daemon, CLI, charter packs, Loom providers (Ollama +
...
> Hands (`helix hands run`), Atlas pins (`helix atlas`), plot commits
> (`helix plot commit|list|rewind|status`), and plot-scoped file tools
> (`helix files ls|read|write|delete`) work today. Mail and desktop are next.
...
### 11. Loom (model providers)

Ask completions go through `helix-loom`. Default is Ollama on loopback. Optional
OpenAI-compatible cloud (workshop only, after Switch allowlist + Reliquary key):

```bash
# Local (default)
helix ask "hello"

# Cloud path (requires workshop pack, seeded Switch host, and a Reliquary key)
helix secrets add openai-key   # value via stdin
export HELIX_PACK=workshop
export HELIX_LOOM=openai
export HELIX_OPENAI_BASE=https://api.openai.com/v1
export HELIX_OPENAI_KEY_REF=openai-key
export HELIX_MODEL=gpt-4o-mini
# Note: until a host is seeded on Switch, cloud dials remain denied.
```
...
### 13. Plot commits (content-addressed snapshots)

Each plot under `~/Helix/plots/<id>/` can be snapshotted. The commit id is the
SHA-256 of a sorted manifest (relative path + per-file digest). Bookkeeping
under `.helix/` is excluded from the tree.

```bash
# After editing files in the default plot:
helix plot commit -m "notes before refactor"
helix plot list
helix plot status
# Restore earlier tree (full id or unique prefix):
helix plot rewind abcdef012345
```

Snapshots live under `plots/<id>/.helix/commits/<sha256>/` (meta.json + tree/).
`HEAD` records the last commit id. Rewind replaces the working tree and updates
HEAD; it does not delete other commits.

### 14. Plot-scoped file tools

The first native adapter is **plot-only** filesystem access. Paths are resolved
under the active plot (same directory Hands mounts at guest `/plot`). Escapes
(`..`, absolute paths, `.helix`) are rejected. There is no host shell.

```bash
helix files write notes/todo.md --content "- ship files adapter"
helix files ls
helix files ls notes
helix files read notes/todo.md
helix files delete notes/todo.md
# stdin also works:
echo "draft" | helix files write notes/draft.md
```

These ops share the plot boundary with Wasm Hands; they do not dial the network
or read outside `~/Helix/plots/<id>/`.
...
| `HELIX_LOOM` | `auto` | Provider: `ollama`, `openai`, or `auto` |
| `HELIX_OPENAI_BASE` | (unset) | OpenAI-compat base URL |
| `HELIX_OPENAI_KEY_REF` | (unset) | Reliquary secret *name* for API key |
...
- Plot commits: `helix plot commit|list|rewind|status` (content-addressed
  snapshots under `plots/<id>/.helix/commits/`)
- Plot-scoped file tools: `helix files ls|read|write|delete` (plot boundary only;
  no host shell)
...
- Seeded remote hosts for cloud Loom (OpenAI path ready; hosts empty until seeded)
...
helix/
  crates/
    ...
    helix-memory/     home layout + retrieval + plot files
